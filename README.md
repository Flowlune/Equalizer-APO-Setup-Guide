# 🎤 Equalizer APO Microphone Setup Guide

Подробная инструкция по установке и настройке **Equalizer APO** для микрофона с использованием **VST-плагинов**.
...
Это помогает развитию репозитория и делает его более заметным для других пользователей.
...
Перед установкой Equalizer APO **обязательно** выполни эти простые рекомендации. Это гарантирует, что твой микрофон будет работать на максимальной мощности и не будет «резать» или «приглушать».

### 1. Настройка чувствительности микрофона
- Если у твоего микрофона есть физический регулятор чувствительности — **поставь его на максимум** (12–16 часов или 100%).
- Если регулятора нет на самом микрофоне — настрой уровень в системе.

### 2. Настройка уровня записи в системе Windows (рекомендуемый уровень 75–80%)

1. На панели задач в правом нижнем углу нажми правой кнопкой мыши на иконку **громкости** (🔊).
2. Выбери пункт **Открыть настройки громкости**.
3. Нажми **Дополнительные параметры громкости**.
4. В правом столбце найди свой микрофон (например, «fiffine Microphone», «RØDE», «HyperX», «Audio-Technica» и т.д.).
5. **Сними галочку** с пункта **«Отключить монопольный режим»** (это важно!).
6. Поставь уровень громкости на **75–80%** (рекомендуется 78–80%).

### Как быстро открыть окно «Уровни» микрофона:
1. Нажми правой кнопкой мыши на иконку 🔊 в трее.
2. Выбери **Звуки**.
3. Перейди во вкладку **Запись**.
4. Правой кнопкой мыши кликни по своему микрофону.
5. Выбери **Свойства** → вкладка **Уровни**.

>[!IMPORTANT]
>### 3. Дополнительные важные настройки
>- В окне **Свойства микрофона** открой вкладку **Улучшения** и поставь флажок **Компенсация громкости** (если есть опция).
>- В **Формат** установи **16 бит, 48000 Гц** (если это возможно для твоего микрофона).

> [!TIP]
>### 4. Важные советы
>- После всех изменений сделай тестовую запись (например, в Discord или OBS) и убедись, что звук чистый.
>- Если звук стал «хрустящим» или «заикается» — попробуй чуть снизить уровень до 70–75%.
>- Если микрофон подключён через интерфейс (Focusrite, Audient, Apogee и т.д.) — проверь, не стоит ли там отдельный регулятор на самом устройстве.

**Готово!** Теперь микрофон будет работать максимально качественно, и Equalizer APO сможет правильно его обрабатывать.
---
...
выберите свой микрофон.

Если всё работает нормально — остальные галочки менять **не нужно**.

После выбора микрофона нажмите **OK**.

> [!IMPORTANT]
> Выбирайте только тот микрофон, который будете использовать.
...
поставьте галочку только возле своего микрофона.
...
<img width="1021" height="327" alt="15  Чтобы добавить конфигурацию нужно нажать на Control далее Include" src="https://github.com/user-attachments/assets/e76186e6-584a-4f71-ab29-c19b6db3f7b0" />
...
---
<img width="491" height="52" alt="16  Далее нужно нажать синию папку и в самой папке уже выбрать конфигурацию" src="https://github.com/user-attachments/assets/09b07085-7e8c-4113-a450-41834b74254a" />

Нажмите кнопку с синей папкой.
...
В папке
<img width="1190" height="739" alt="17  В папке Config нужно выбрать конфигурацию  txt и нажать на кнопку Открыть" src="https://github.com/user-attachments/assets/605ad650-391f-46bb-8d63-2d28914e4535" />
```
Config
```
...
---
<img width="1116" height="478" alt="18  Чтобы добавлять  txt конфигурации нужно зайти в папку Config" src="https://github.com/user-attachments/assets/a6e0b27a-05cc-4699-8b1a-f13985dc532e" />

> [!NOTE]
> Все дополнительные конфигурации (.txt) необходимо помещать в папку:
>
> ```
> C:\Program Files\EqualizerAPO\config
> ```
...
> [!CAUTION]
> Не удаляйте системные файлы Equalizer APO из папки установки.
> Удаляйте или изменяйте только те конфигурации и плагины, которые добавляли самостоятельно.

---
<img width="511" height="401" alt="Настройки у vst плагина reagate-standalone" src="https://github.com/user-attachments/assets/76aceb8b-94a7-45f9-94db-c8b7b9802fbd" />
...
> [!NOTE]
> Конфигурации Equalizer APO имеют расширение **.txt**.
...
Хочешь сразу протестировать ReaGate и LoudMax без создания своего конфига?  
Скачай готовый файл ниже и замени им существующий `config.txt`.
...
> [!IMPORTANT]
>**Если Google drive либо Yandex Disk не работают** — попробуй открыть в браузере через инкогнито или используй VPN.
---
